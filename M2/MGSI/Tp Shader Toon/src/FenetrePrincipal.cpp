...
FenetrePrincipal::FenetrePrincipal(int *argc, char **argv, Scene *s, SceneControleur *c) : scene(s), controleur(c)
{
	...
	fenetre = glfwCreateWindow(800, 600, "Shader", NULL, NULL);
    ...
    uniformShader.push_back("scaleMin");
    uniformShader.push_back("scaleMax");
    uniformShader.push_back("angleMin");
    uniformShader.push_back("angleMax");
...
void FenetrePrincipal::Affichage(){                       
    ...
    GLuint idSMax = scene->GetShader()->getLocation("scaleMax");
    GLuint idSMin = scene->GetShader()->getLocation("scaleMin");
    GLuint idAMax = scene->GetShader()->getLocation("angleMax");
    GLuint idAMin = scene->GetShader()->getLocation("angleMin");
    ...
    glUniform1f(idSMax,scaleMax);
    glUniform1f(idSMin,scaleMin);

    glUniform1f(idAMax,angleMax);
    glUniform1f(idAMin,angleMin);
...
void FenetrePrincipal::Clavier(GLFWwindow* window, int key, int scancode, int action, int mods){
    if(action == GLFW_PRESS){
        ...
        switch(key){
            ...
            case GLFW_KEY_UP: /* affichage du carre plein */
                materialShininess += 0.5;
            break;
            case GLFW_KEY_DOWN: /* affichage du carre plein */
                materialShininess -= 0.5;
            break;

            case GLFW_KEY_LEFT: /* affichage du carre plein */
                LightAmbientCoefficient += 0.1;
            break;
            case GLFW_KEY_RIGHT: /* affichage du carre plein */
                LightAmbientCoefficient -= 0.1;
            break;

            case GLFW_KEY_A: /* affichage du carre plein */
                angleMax += 0.1;    

                if(angleMax < 0)
                    angleMax = -1.0*angleMax;

                if(angleMax > 3.14)
                    angleMax = 0;

            break;
            case GLFW_KEY_Z: /* affichage du carre plein */
                angleMin += 0.1;    

                if(angleMin < 0)
                    angleMin = -1.0*angleMin;

                if(angleMin > 3.14)
                    angleMin = 0;
            break;

            case GLFW_KEY_Q: /* affichage du carre plein */
                scaleMax += 0.1;    
                ...
                if(scaleMax > 1.0)
                    scaleMax = 0.1;
            break;
            case GLFW_KEY_S: /* affichage du carre plein */
                scaleMin += 0.1;    
