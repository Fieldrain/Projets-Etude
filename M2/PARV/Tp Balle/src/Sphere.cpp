...
Sphere::Sphere(double* Origin,float Diametre,int Parallele, int Meridien) : Object(),
    parallele(Parallele),
    meridien(Meridien),
    diametre(Diametre),
    origin(Origin)
...
void Sphere::initSommet(){
    ...
    for (int i = 0; i < meridien; i++) {
        ...
        for (int j = 0; j < parallele; j++) {
            ...
            value[0] = origin[0] + (rayon * (cos(y) * sin(o))) ;
            value[1] = origin[1] + (rayon * sin(y));
            value[2] = origin[2] + (rayon * (cos(o) * cos(y)));
    ...
    valueH[0] = origin[0] + 0.0;
    valueH[1] = origin[1] + rayon;
    valueH[2] = origin[2] + 0.0;
    ...
    valueB[0] = origin[0] +  0.0;
    valueB[1] = origin[1] + -rayon;
    valueB[2] = origin[2] + 0.0;
