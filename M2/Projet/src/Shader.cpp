...
///
bool Shader::loadShaderFile(std::string strFilename, GLuint iHandle)
{
...
void Shader::recompileShader(std::string strFileName, SHADERTYPE typeShader)
{
...
void Shader::loadShader(std::string strFileName, SHADERTYPE typeShader)
{
	GLuint hShader = 0;
	GLint status;
...
void Shader::addAttribute(std::string strParName)
{
	m_mapVarShader[strParName] = glGetAttribLocation(m_uIdProgram, strParName.c_str());
}

void Shader::addAttribute(GLuint index,std::string strParName){
	m_mapVarShader[strParName] = index;
	glBindAttribLocation(m_uIdProgram,index,strParName.c_str());
}

GLint Shader::getLocation(std::string strParName)
{
...
void Shader::setSubroutine(std::string strUniformName, std::string strSubRoutine, int iShaderType)
{
